...
class BacteriaQuery:
    ...
    def createQuery(self):
        return self.prefix + "esearch.fcgi?db={0}&term={1}+AND+{2}+NOT+{3}+NOT+{4}&retstart=0&retmax=5000".format("pubmed", self.term_one, self.term_two, "virus", "viral")
...
def BacteriaDatasetQuery(uid):
    try:
        r = requests.get("https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db={0}&id={1}&retmode=text&rettype=abstract&api_key=49c77251ac91cbaa16ec5ae4269ab17d9d09".format("pubmed", uid))
        f = open("/Users/rahulnatarajan/Research/Garud/bacteria/{0}.txt".format(str(uid)), "w")
        f.write(r.text)
        f.close()
        logger.info('Added abstracts related to {0}'.format(uid))
    except requests.HTTPError as e:
...
def BacteriaPredictionDatasetQuery(uid, set_of_valid_ids, testing_threshold, validation_threshold):
    try:
        ...
        if uid in set_of_valid_ids:
            if dataset <= testing_threshold:
                f = open("/Users/rahulnatarajan/Research/Garud/train/updated_bacteria_test/pos/{0}.txt".format(str(uid)), "w")
                f.write(r.text)
                f.close()
            elif dataset <= testing_threshold + validation_threshold:
                f = open("/Users/rahulnatarajan/Research/Garud/train/updated_bacteria_validation/pos/{0}.txt".format(str(uid)), "w")
                f.write(r.text)
                f.close()
            else:
                f = open("/Users/rahulnatarajan/Research/Garud/train/updated_bacteria_train/pos/{0}.txt".format(str(uid)), "w")
                f.write(r.text)
                f.close()
            logger.info('Added abstracts related to {0}'.format(uid))
        else:
            if dataset <= testing_threshold:
                f = open("/Users/rahulnatarajan/Research/Garud/train/updated_bacteria_test/neg/{0}.txt".format(str(uid)), "w")
                f.write(r.text)
                f.close()
            elif dataset <= testing_threshold + validation_threshold:
                f = open("/Users/rahulnatarajan/Research/Garud/train/updated_bacteria_validation/neg/{0}.txt".format(str(uid)), "w")
                f.write(r.text)
                f.close()
            else:
                f = open("/Users/rahulnatarajan/Research/Garud/train/updated_bacteria_train/neg/{0}.txt".format(str(uid)), "w")
                f.write(r.text)
                f.close()
            logger.info('Added abstracts related to {0}'.format(uid))
    except requests.HTTPError as e:
